...
def process_uploaded_files(files):
    ...
    for uploaded_file in files:
        ...
        # 🚨 If no text → use OCR
        if len(docs) == 0:
            st.warning(f"⚠️ Using OCR for {uploaded_file.name} (scanned PDF)")
...
if st.button("Process Documents"):
    ...
    with st.spinner("Processing PDFs..."):
        ...
        st.write(f"✂️ Chunks created: {len(final_docs)}")
...
# Query input
user_prompt = st.text_input("Ask a question from your PDFs")
